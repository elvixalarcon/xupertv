...
function IconShuffle({ on }) {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" className={on ? 'sp-icon on' : 'sp-icon'}>
...
export default function BottomPlayer({ onOpenNowPlaying }) {
  ...
  return (
    <footer className={`player-bar ${track ? '' : 'player-bar--idle'}`}>
      {track && (
        <div className="player-bar__top-progress" aria-hidden>
          <div className="player-bar__top-progress-fill" style={{ width: `${pct}%` }} />
        </div>
      )}
      ...
      <div className="player-bar__left">
        ...
        {track && (
          <button type="button" className="player-bar__heart player-bar__heart--desktop" onClick={onFav} title="Favorito">
            {isFavorite(track.id) ? '♥' : '♡'}
          </button>
        )}
        {track && <DownloadButton track={track} className="player-bar__dl player-bar__dl--desktop" />}
      </div>

      <div className="player-bar__center player-bar__center--desktop">
        ...
        <div className="player-bar__buttons">
          <button type="button" className={shuffle ? 'sp-btn on' : 'sp-btn'} onClick={() => setShuffle(!shuffle)} title="Aleatorio">
            <IconShuffle on={shuffle} />
          </button>
          <button type="button" className={radioMode ? 'sp-btn on' : 'sp-btn'} onClick={() => setRadioMode(!radioMode)} title="Radio">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm0 1.5a5.5 5.5 0 1 1 0 11 5.5 5.5 0 0 1 0-11zm-.75 2.5v5l4.5 2.6.75-1.3-3.75-2.15V5z" /></svg>
          </button>
          <button type="button" className="sp-btn" onClick={prev} disabled={!track} aria-label="Anterior">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M3.3 1a.7.7 0 0 1 .7.7v5.15l9.95-5.744a.7.7 0 0 1 1.05.606v12.575a.7.7 0 0 1-1.05.607L4 9.149V14.3a.7.7 0 0 1-1.4 0V1.7a.7.7 0 0 1 .7-.7z" /></svg>
          </button>
          <button type="button" className="sp-btn-play" onClick={toggle} disabled={!track} aria-label={playing ? 'Pausa' : 'Play'}>
            {playing ? (
              <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M2.7 1a.7.7 0 0 0-.7.7v12.6a.7.7 0 0 0 1.4 0V1.7a.7.7 0 0 0-.7-.7zm5 0a.7.7 0 0 0-.7.7v12.6a.7.7 0 0 0 1.4 0V1.7a.7.7 0 0 0-.7-.7zm5 0a.7.7 0 0 0-.7.7v12.6a.7.7 0 0 0 1.4 0V1.7a.7.7 0 0 0-.7-.7z" /></svg>
            ) : (
              <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M3 1.713a.7.7 0 0 0-1.05.607v12.575a.7.7 0 0 0 1.075.618l9.671-5.825a.7.7 0 0 0 0-1.234L3 1.713z" /></svg>
            )}
          </button>
          <button type="button" className="sp-btn" onClick={next} disabled={!track} aria-label="Siguiente">
            <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M12.7 1a.7.7 0 0 0-.7.7v5.15L2.05 1.107A.7.7 0 0 0 1 1.712v12.575a.7.7 0 0 0 1.05.607L12 9.149V14.3a.7.7 0 0 0 1.4 0V1.7a.7.7 0 0 0-.7-.7z" /></svg>
          </button>
      ...
      <div className="player-bar__right">
        {track && (
          <button
            type="button"
            className={`player-bar__heart player-bar__heart--mobile ${isFavorite(track.id) ? 'on' : ''}`}
            onClick={onFav}
            title="Favorito"
          >
            {isFavorite(track.id) ? '♥' : '♡'}
        ...
        {track && (
          <button
            type="button"
            className="sp-btn-play player-bar__play-mobile"
            onClick={toggle}
            disabled={!track}
        ...
        {track && (
          <button
            type="button"
            className={`player-bar__video-btn player-bar__video-btn--desktop ${videoPreview ? 'on' : ''}`}
