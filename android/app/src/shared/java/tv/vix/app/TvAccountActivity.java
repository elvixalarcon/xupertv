    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.tv_hub_functions).setOnClickListener(v -> {
            if ("tv".equals(BuildConfig.PLATFORM)) {
                try {
                    Class<?> cls = Class.forName("tv.vix.app.TvFunctionsActivity");
                    startActivity(new Intent(this, cls));
                } catch (ClassNotFoundException ignored) { /* solo TV */ }
            }
        });
        ...
        findViewById(R.id.tv_account_change_password).setOnClickListener(v -> {
            Intent i = new Intent(this, TvChangePasswordActivity.class);
            String user = accountId.getText() != null ? accountId.getText().toString() : "";
            if (!user.isEmpty() && !"Mi cuenta".equals(user)) {
                i.putExtra(TvChangePasswordActivity.EXTRA_USERNAME, user);
            }
            startActivity(i);
        });
